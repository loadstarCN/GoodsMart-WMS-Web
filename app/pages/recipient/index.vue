...
const deleteItem = async (item_id:Number) => {
    ...
    if (confirm) {
        await httpRequest(`/api/warehouse/recipient/${item_id}`, {
            method: 'DELETE',
            onSuccess: async() => {
            ...
            <div class="card custom-card">
                <div class="card-header justify-content-between">
                    <div class="card-title">
                        {{ $t('recipient.title') }} <span class="badge bg-light text-default rounded ms-1 fs-12 align-middle">{{ pageData?.total }}</span>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table text-nowrap table-bordered table-hover">
                            ...
                            <tbody>
                                <tr v-for="recipient in pageData?.items" class="invoice-list">
                                    <td class="d-none d-md-table-cell">{{ recipient?.id }}</td>
                                    <td>
                                      <NuxtLink :to="`/recipient/detail/${recipient?.id}`">{{ recipient?.name }}</NuxtLink>
                                    </td>
                                    <td class="d-none d-xxl-table-cell">{{ $dayjs(recipient?.updated_at) }}</td>
