...
const deleteItem = async (item_id:Number) => {
    ...
    if (confirm) {
        await httpRequest(`/api/warehouse/carrier/${item_id}`, {
            method: 'DELETE',
            onSuccess: async() => {
            ...
            <div class="card custom-card">
                <div class="card-header justify-content-between">
                    <div class="card-title">
                        {{ $t('carrier.title') }} <span class="badge bg-light text-default rounded ms-1 fs-12 align-middle">{{ pageData?.total }}</span>
                    </div>
                    <div class="d-flex flex-wrap gap-2">
                        <div class="d-flex flex-wrap gap-2" >
                            <NuxtLink to="/carrier/add" class="btn btn-primary btn-wave"><i class="ri-add-line me-1 fw-semibold align-middle"></i>{{ $t('carrier.operations.add') }}</NuxtLink>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table text-nowrap table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <tr v-for="carrier in pageData?.items" class="invoice-list">
                                    <td class="d-none d-md-table-cell">{{ carrier?.id }}</td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="hstack gap-2 fs-15">                                            
                                            <NuxtLink :to="`/carrier/edit/${carrier?.id}`" class="btn btn-icon btn-sm btn-success-light product-btn"><i class="ri-edit-line" ></i></NuxtLink>
                                           
                                            <NuxtLink href="javascript:void(0);" @click="inactiveItem(carrier.id)" class="btn btn-icon btn-sm btn-warning-light product-btn" v-if="activeTab === 'active'"><i class="ri-eye-off-line"></i></NuxtLink>
                                            <NuxtLink href="javascript:void(0);" @click="activeItem(carrier.id)" class="btn btn-icon btn-sm btn-warning-light product-btn" v-else><i class="ri-eye-line"></i></NuxtLink>
                                            <NuxtLink href="javascript:void(0);" @click="deleteItem(carrier.id)" class="btn btn-icon btn-sm btn-danger-light product-btn"><i class="ri-delete-bin-line"></i></NuxtLink>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
