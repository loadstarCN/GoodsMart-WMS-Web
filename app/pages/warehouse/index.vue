...
const fetchData = async () => {
    loading.value = true;
    const data = await httpRequest<PaginationData>('/api/warehouse/warehouse/', {
        method: 'GET',
        params: route.query,
...
const deleteItem = async (item_id:Number) => {
    ...
    if (confirm) {
        await httpRequest(`/api/warehouse/warehouse/${item_id}`, {
            method: 'DELETE',
            onSuccess: async() => {
            ...
            <div class="card custom-card">
                <div class="card-header justify-content-between">
                    <div class="card-title">
                        {{ $t('warehouse.title') }} <span class="badge bg-light text-default rounded ms-1 fs-12 align-middle">{{ pageData?.total }}</span>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table text-nowrap table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <tr v-for="warehouse in pageData?.items" class="invoice-list">
                                    <td class="d-none d-md-table-cell">{{ warehouse?.id }}</td>
                                    <td>
                                      <NuxtLink :to="`/warehouse/detail/${warehouse?.id}`">{{ warehouse?.name }}</NuxtLink>
                                    </td>
                                    <td class="d-none d-xxl-table-cell">{{ $dayjs(warehouse?.updated_at) }}</td>
