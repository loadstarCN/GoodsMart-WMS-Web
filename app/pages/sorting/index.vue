...
const fetchData = async () => {
    loading.value = true;
    const data = await httpRequest<PaginationData>('/api/warehouse/sorting/', {
        method: 'GET',
        params: route.query,
...
const fetchMonthlyStatsData = async () => {
    loading.value = true;
    const data = await httpRequest('/api/warehouse/sorting/monthly-stats', {
        method: 'GET',
        params: {'months': 6},
        onSuccess: async(data) => {
            // 处理数据格式转换
...
const fetchStatsData = async () => {
    loading.value = true;
    const data = await httpRequest('/api/warehouse/sorting/status-overview-stats', {
        method: 'GET',
        params: route.query,
        onSuccess: async(data) => {
                ...
                <div class="card-header justify-content-between">
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <div class="d-flex" role="search">
                            <input class="form-control me-2" type="search" :placeholder="t('sorting.form.placeholders.search')"
                        ...
                        <table class="table text-nowrap table-bordered">
                            ...
                            <tbody>
                                <tr v-for="item in pageData?.items" class="invoice-list">
                                    <td class="">
                                        <NuxtLink :to="`/sorting/detail/${item?.id}`" class="text-default">SORT-{{ item?.id }}
                                        </NuxtLink>
                                    </td>
                                    <td class=""><NuxtLink :to="`/asn/detail/${item?.asn_id}`" class="text-default">ASN-{{ item?.asn_id }}</NuxtLink>
                                    </td>
