            ...
            <div class="card custom-card">
                <div class="card-header justify-content-between">
                    <div class="card-title">
                        {{ $t('supplier.title') }} <span class="badge bg-light text-default rounded ms-1 fs-12 align-middle">{{ pageData?.total }}</span>
                    </div>
                    <div class="d-flex flex-wrap gap-2">
                        <div class="d-flex flex-wrap gap-2" >
                            <NuxtLink to="/supplier/add" class="btn btn-primary btn-wave"><i class="ri-add-line me-1 fw-semibold align-middle"></i>{{t('supplier.operations.add')}}</NuxtLink>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table text-nowrap table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <tr v-for="supplier in pageData?.items" class="invoice-list">
                                    <td class="d-none d-md-table-cell">{{ supplier?.id }}</td>
                                    <td>
                                      <NuxtLink :to="`/supplier/detail/${supplier?.id}`">{{ supplier?.name }}</NuxtLink>
                                    </td>
                                    <td class="d-none d-xxl-table-cell">{{ $dayjs(supplier?.updated_at) }}</td>
                                    <td class="d-none d-xxl-table-cell">{{ supplier?.creator?.user_name }}</td>
                                    <td>
                                        <div class="hstack gap-2 fs-15">                                            
                                            <NuxtLink :to="`/supplier/edit/${supplier?.id}`" class="btn btn-icon btn-sm btn-success-light product-btn"><i class="ri-edit-line" ></i></NuxtLink>
                                           
                                            <NuxtLink href="javascript:void(0);" @click="inactiveItem(supplier.id)" class="btn btn-icon btn-sm btn-warning-light product-btn" v-if="activeTab === 'active'"><i class="ri-eye-off-line"></i></NuxtLink>
                                            <NuxtLink href="javascript:void(0);" @click="activeItem(supplier.id)" class="btn btn-icon btn-sm btn-warning-light product-btn" v-else><i class="ri-eye-line"></i></NuxtLink>
                                            <NuxtLink href="javascript:void(0);" @click="deleteItem(supplier.id)" class="btn btn-icon btn-sm btn-danger-light product-btn"><i class="ri-delete-bin-line"></i></NuxtLink>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
