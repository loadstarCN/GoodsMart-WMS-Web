    ...
    const data = await httpRequest(`/api/warehouse/supplier/${itemId}`, {
        ...
        onError: (error) => {
            if (error.status === 404) {
                ...
                showError({ statusCode: 404, message: t('action-results.not-found',{entity:t('common.entities.supplier'),identifier:itemId}) });
            }else {
    ...
    <div class="col-xl-12">
      <div class="card custom-card">
        ...
        <div class="px-4 py-3 border-top border-block-start-dashed d-sm-flex justify-content-end">
          <NuxtLink :to="`/supplier/edit/${itemId}`" class="btn btn-primary mb-2">{{t('supplier.operations.edit')}}
          </NuxtLink>
        </div>
      </div>
    </div>
