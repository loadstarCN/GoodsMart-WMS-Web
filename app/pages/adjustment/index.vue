...
const fetchData = async () => {
    loading.value = true;
    const data = await httpRequest<PaginationData>('/api/warehouse/adjustment/', {
        method: 'GET',
        params: route.query,
...
const deleteItem = async (item_id:Number) => {
    ...
    if (confirm) {
        await httpRequest(`/api/warehouse/adjustment/${item_id}`, {
            method: 'DELETE',
            onSuccess: async() => {
...
const fetchMonthlyStatsData = async () => {
    loading.value = true;
    const data = await httpRequest('/api/warehouse/adjustment/monthly-stats', {
        method: 'GET',
        params: {'months': 6},
        onSuccess: async(data) => {
            // 处理数据格式转换
...
const fetchStatsData = async () => {
    loading.value = true;
    const data = await httpRequest('/api/warehouse/adjustment/status-overview-stats', {
        method: 'GET',
        params: route.query,
        onSuccess: async(data) => {
                ...
                <div class="card-header justify-content-between">
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <div class="d-flex" role="search">
                            <input class="form-control me-2" type="search" :placeholder="t('adjustment.form.placeholders.search')"
                        ...
                        <ul class="nav nav-pills nav-style-3 mb-3" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link" :class="{ active: activeTab === 'approved' }"
                                    href="javascript:void(0);" @click="setActiveFilter('approved')">{{t('common.status.approved')}}</a>
                            </li>
                            <li class="nav-item">
                        ...
                        <table class="table text-nowrap table-bordered">
                            ...
                            <tbody>
                                <tr v-for="item in pageData?.items" class="invoice-list">
                                    <td class="">
                                        <NuxtLink :to="`/adjustment/detail/${item?.id}`" class="text-default">ADJ-{{ item?.id }}
                                        </NuxtLink>
                                    </td>
