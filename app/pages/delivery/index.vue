...
const fetchData = async () => {
    loading.value = true;
    const data = await httpRequest<PaginationData>('/api/warehouse/delivery/', {
        method: 'GET',
        params: route.query,
...
//     if (confirm) {
//         await httpRequest(`/api/warehouse/delivery/${item_id}`, {
//             method: 'DELETE',
//             onSuccess: async() => {
...
const fetchMonthlyStatsData = async () => {
    loading.value = true;
    const data = await httpRequest('/api/warehouse/delivery/monthly-stats', {
        method: 'GET',
        params: {'months': 6},
        onSuccess: async(data) => {
            // 处理数据格式转换
...
const fetchStatsData = async () => {
    loading.value = true;
    const data = await httpRequest('/api/warehouse/delivery/status-overview-stats', {
        method: 'GET',
        params: route.query,
        onSuccess: async(data) => {
                ...
                <div class="card-header justify-content-between">
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <div class="d-flex" role="search">
                            <input class="form-control me-2" type="search" :placeholder="t('delivery.form.placeholders.search')"
                ...
                <div class="card-body">
                    <div>
                        <ul class="nav nav-pills nav-style-3 mb-3" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link" :class="{ active: activeTab === 'signed' }"
                                    href="javascript:void(0);" @click="setActiveFilter('signed')">{{t('common.status.signed')}}</a>
                            </li>
                        </ul>
                    </div>
                        ...
                        <table class="table text-nowrap table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">DN</th>
                                    <th scope="col" class="d-none d-md-table-cell">{{t('dn.fields.scheduled-date')}}</th>
                                    <th scope="col" class="d-none d-xl-table-cell">{{t('common.entities.recipient')}}</th>
                                    <th scope="col" class="d-none d-xl-table-cell">{{t('common.entities.carrier')}}</th>
                            ...
                            <tbody>
                                <tr v-for="item in pageData?.items" class="invoice-list">
                                    <td class="">
                                        <NuxtLink :to="`/delivery/detail/${item?.id}`" class="text-default">DEL-{{ item?.id }}
                                        </NuxtLink>
                                    </td>
                                    <td class=""><NuxtLink :to="`/dn/detail/${item?.dn_id}`" class="text-default">DN-{{ item?.dn_id }}</NuxtLink>
                                    </td>
                                    <td class="d-none d-xl-table-cell">{{ item?.expected_shipping_date }}</td>
                                    <td class="d-none d-xl-table-cell">{{ item?.recipient_name }}</td>
                                    <td class="d-none d-xl-table-cell">{{ item.carrier_name }}</td>
