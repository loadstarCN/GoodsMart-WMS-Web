...
const deleteItem = async (item_id:Number) => {
    ...
    if (confirm) {
        await httpRequest(`/api/warehouse/staff/${item_id}`, {
            method: 'DELETE',
            onSuccess: async() => {
            ...
            <div class="card custom-card">
                <div class="card-header justify-content-between">
                    <div class="card-title">
                        {{ $t('staff.title') }} <span class="badge bg-light text-default rounded ms-1 fs-12 align-middle">{{ pageData?.total }}</span>
                    </div>
                    <div class="d-flex flex-wrap gap-2">
                        <div class="d-flex flex-wrap gap-2" >
                            <NuxtLink to="/staff/add" class="btn btn-primary btn-wave"><i class="ri-add-line me-1 fw-semibold align-middle"></i>{{ t('staff.operations.add') }}</NuxtLink>
                        </div>
                        ...
                        <table class="table text-nowrap table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" class="d-none d-xxl-table-cell">{{ t('staff.fields.department') }}</th>
                                    <th scope="col" class="d-none d-xxl-table-cell">{{ t('staff.fields.position') }}</th>
                                    <th scope="col" class="d-none d-xxl-table-cell">{{ t('staff.fields.employee-number') }}</th>
                                    <th scope="col" class="d-none d-xxl-table-cell">{{ t('staff.fields.hire-date') }}</th>
                            ...
                            <tbody>
                                ...
                                <tr v-for="staff in pageData?.items" class="invoice-list">
                                    <td class="d-none d-md-table-cell">{{ staff?.id }}</td>
                                    <td>
                                      <NuxtLink :to="`/staff/detail/${staff?.id}`">{{ staff?.user_name }}</NuxtLink>
                                    </td>
                                    <td class="d-none d-xxl-table-cell">{{ staff?.department?.name }}</td>
                                    <td class="d-none d-xxl-table-cell">{{ staff?.position }}</td>
                                    <td class="d-none d-xxl-table-cell">{{ staff?.employee_number }}</td>
                                    <td class="d-none d-xxl-table-cell">{{ $dayjs(staff?.hire_date,'YYYY-MM-DD') }}</td>
                                    <td class="d-none d-xxl-table-cell">{{ $dayjs(staff?.updated_at) }}</td>
                                    <td class="d-none d-xxl-table-cell">{{ staff?.creator?.user_name }}</td>
                                    <td>
                                        <div class="hstack gap-2 fs-15">                                            
                                            <NuxtLink :to="`/staff/edit/${staff?.id}`" class="btn btn-icon btn-sm btn-success-light product-btn"><i class="ri-edit-line" ></i></NuxtLink>
                                           
                                            <NuxtLink href="javascript:void(0);" @click="inactiveItem(staff.id)" class="btn btn-icon btn-sm btn-warning-light product-btn" v-if="activeTab === 'active'"><i class="ri-eye-off-line"></i></NuxtLink>
                                            <NuxtLink href="javascript:void(0);" @click="activeItem(staff.id)" class="btn btn-icon btn-sm btn-warning-light product-btn" v-else><i class="ri-eye-line"></i></NuxtLink>
                                            <NuxtLink href="javascript:void(0);" @click="deleteItem(staff.id)" class="btn btn-icon btn-sm btn-danger-light product-btn"><i class="ri-delete-bin-line"></i></NuxtLink>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
