...
const fetchData = async () => {
    loading.value = true;
    const data = await httpRequest<PaginationData>('/api/warehouse/picking/', {
        method: 'GET',
        params: route.query,
...
//     if (confirm) {
//         await httpRequest(`/api/warehouse/picking/${item_id}`, {
//             method: 'DELETE',
//             onSuccess: async() => {
...
const fetchMonthlyStatsData = async () => {
    loading.value = true;
    const data = await httpRequest('/api/warehouse/picking/monthly-stats', {
        method: 'GET',
        params: {'months': 6},
        onSuccess: async(data) => {
            // 处理数据格式转换
...
const fetchStatsData = async () => {
    loading.value = true;
    const data = await httpRequest('/api/warehouse/picking/status-overview-stats', {
        method: 'GET',
        params: route.query,
        onSuccess: async(data) => {
                ...
                <div class="card-header justify-content-between">
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <div class="d-flex" role="search">
                            <input class="form-control me-2" type="search" :placeholder="t('picking.form.placeholders.search')"
                        ...
                        <table class="table text-nowrap table-bordered">
                            ...
                            <tbody>
                                <tr v-for="item in pageData?.items" class="invoice-list">
                                    <td class="">
                                        <NuxtLink :to="`/picking/detail/${item?.id}`" class="text-default">PICK-{{ item?.id }}
                                        </NuxtLink>
                                    </td>
                                    <td class=""><NuxtLink :to="`/dn/detail/${item?.dn_id}`" class="text-default">DN-{{ item?.dn_id }}</NuxtLink>
                                    </td>
                                    <td class="d-none d-xl-table-cell">{{ $n(item?.expected_quantity) }} PCS</td>
                                    <td class="d-none d-md-table-cell">{{ item?.expected_shipping_date }}</td>
