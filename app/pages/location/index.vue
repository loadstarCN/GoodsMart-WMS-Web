...
async function search() {
    await router.push({ query: { ...route.query, code: keyword.value.trim(),page:1 } });
}
onMounted(async() => {
    fetchData();
});
...
const deleteItem = async (item_id:Number) => {
    ...
    if (confirm) {
        await httpRequest(`/api/warehouse/location/${item_id}`, {
            method: 'DELETE',
            onSuccess: async() => {
            ...
            <div class="card custom-card">
                <div class="card-header justify-content-between">
                    <div class="card-title">
                        {{ $t('location.title') }} <span class="badge bg-light text-default rounded ms-1 fs-12 align-middle">{{ pageData?.total }}</span>
                    </div>
                    <div class="d-flex flex-wrap gap-2">
                        <div class="d-flex flex-wrap gap-2" >
                            <NuxtLink to="/location/add" class="btn btn-primary btn-wave"><i class="ri-add-line me-1 fw-semibold align-middle"></i>{{t('location.operations.add')}}</NuxtLink>
                        </div>
                        ...
                        <table class="table text-nowrap table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">{{ $t('common.fields.code') }}</th>
                                    <th scope="col" class="d-none d-lg-table-cell">{{ t('location.fields.location-type') }}</th>
                                    <th scope="col" class="d-none d-md-table-cell">{{ t('location.fields.warehouse') }}</th>
                            ...
                            <tbody>
                                ...
                                <tr v-for="location in pageData?.items" class="invoice-list">
                                    <td class="d-none d-md-table-cell">{{ location?.id }}</td>
                                    <td>
                                      <NuxtLink :to="`/location/detail/${location?.id}`">{{ location?.code }}</NuxtLink>
                                    </td>
                                    <td class="d-none d-lg-table-cell">{{ t('common.locations.'+location?.location_type) }}</td>
                                    <td class="d-none d-md-table-cell"><NuxtLink :to="`/warehouse/detail/${location?.warehouse_id}`">{{ location?.warehouse_name }}</NuxtLink></td>
                                    <td class="d-none d-xxl-table-cell">{{ $dayjs(location?.updated_at) }}</td>
                                    <td class="d-none d-xxl-table-cell">{{ location?.creator?.user_name }}</td>
                                    <td>
                                        <div class="hstack gap-2 fs-15">                                            
                                            <NuxtLink :to="`/location/edit/${location?.id}`" class="btn btn-icon btn-sm btn-success-light product-btn"><i class="ri-edit-line" ></i></NuxtLink>
                                           
                                            <NuxtLink href="javascript:void(0);" @click="inactiveItem(location.id)" class="btn btn-icon btn-sm btn-warning-light product-btn" v-if="activeTab === 'active'"><i class="ri-eye-off-line"></i></NuxtLink>
                                            <NuxtLink href="javascript:void(0);" @click="activeItem(location.id)" class="btn btn-icon btn-sm btn-warning-light product-btn" v-else><i class="ri-eye-line"></i></NuxtLink>
                                            <NuxtLink href="javascript:void(0);" @click="deleteItem(location.id)" class="btn btn-icon btn-sm btn-danger-light product-btn"><i class="ri-delete-bin-line"></i></NuxtLink>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
